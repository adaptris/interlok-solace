...
public class TestBridge implements XMLMessageListener {
  ...
  public TestBridge() {
    unacknowledgedMessages = new HashMap<String, BytesXMLMessage>();
  ...
  public void onReceive(BytesXMLMessage message) {
    log.debug("Received message with ID: {}", message.getMessageId());
        
    unacknowledgedMessages.put(message.getMessageId(), message);
...
//    });
  ...
  private void onMessageDoSend(BytesXMLMessage message) throws Exception {
    ...
    textMessage.setCorrelationKey(message.getMessageId());
    ...
    log.trace("Message with ID {} produced in {}, avg {}", message.getMessageId(), totalTime / MILLION, avgNanos / MILLION);
  ...
  class SolaceProducerSession {
    ...
    void refreshIfNeeded(JCSMPStreamingPublishCorrelatingEventHandler eventHandler) throws Exception {
      if(this.needsRefresh()) {
        ...
        this.producer = this.session.getMessageProducer(eventHandler);
      }
    }
  }
}
